...
elseif isempty(files)
    error('No Excel file found in the current folder');
else
    error('Multiple Excel files found - please specify which one to use');
end

acc_A = Data(1:5001,2)*(2.38419e-6)*4.67; % On the rail feet (north)
acc_B = Data(1:5001,3)*(2.38419e-6)*4.67; % On the slab
acc_C = Data(1:5001,4)*(2.38419e-6)*4.67; % close to the slab

t = 0:1/200:length(acc_A)/200-1/200; %Time
...
highcut = 99.5; % Upper cutoff frequency (Hz)
...
for i = 1:length(PPA_A)
    if fs/5+i < length(PPA_A)
        RMSA(i) = rms(PPA_A(i:fs/5+i));
    else
        RMSA(i) = rms(PPA_A(i-fs/5:i-1));
    end
end

RMSB = zeros(length(PPA_B),1);
for i = 1:length(PPA_B)
    if fs/5+i < length(PPA_B)
        RMSB(i) = rms(PPA_B(i:fs/5+i));
    else
        RMSB(i) = rms(PPA_B(i-fs/5:i-1));
    end
end

RMSC = zeros(length(PPA_C),1);
for i = 1:length(PPA_C)
    if fs/5+i < length(PPA_C)
        RMSC(i) = rms(PPA_C(i:fs/5+i));
    else
        RMSC(i) = rms(PPA_C(i-fs/5:i-1));
    end
end
...
f_center = f_center.';
% Convert to dB scale
PPA_ref = 0.0000254; % Reference PPV for dB conversion, adjust as needed
...
% Maximums
% Time domain ppv
PPA_A_max = max(max(PPA_A),abs(min(PPA_A))); % Max On the rail feet (north)
PPA_B_max = max(max(PPA_B),abs(min(PPA_B))); % Max On the slab
PPA_C_max = max(max(PPA_C),abs(min(PPA_C))); % Max close to the slab
...
% Create the table
T_maximums = table([PPA_A_max;PPA_B_max;PPA_C_max],...
                   [fA_max;fB_max;fC_max],...
                   [P1A_max;P1B_max;P1C_max],...
                   [RMSA_max;RMSB_max;RMSC_max],...
                   'VariableNames',{'Maximum PPV (mm/s)','Frequency (Hz)','Maximum Amplitude (dB)','Maximum RMS (dB)'},...
                   'RowNames',{'D';'E';'F'});


T_frequency_domain = table(f_center,AMP_A_dB,AMP_B_dB,AMP_C_dB,'VariableNames',{'Frequency(Hz)','D','E','F'});


% Plots
fnom = file(1:end-5);
b1 = [fnom 'Time domain E.jpg'];
b2 = [fnom 'Time domain F.jpg'];
b3 = [fnom 'Time domain G.jpg'];

b6 = [fnom 'Frequency domain E.jpg'];
b7 = [fnom 'Frequency domain F.jpg'];
b8 = [fnom 'Frequency domain G.jpg'];

b11 = [fnom 'One-third Octave band E.jpg'];
b12 = [fnom 'One-third Octave band F.jpg'];
b13 = [fnom 'One-third Octave band G.jpg'];
...
ylabel('Peak particle velocity [mm/s]', 'FontName', 'Times New Roman', ...
       'FontSize', 20, 'Color', [0,0,0]);
...
xlabel('1/3 Octave Band Center Frequency [Hz]', 'FontName', 'Times New Roman', ...
       'FontSize', 20, 'Color', [0,0,0]);
ylabel('Amplitude [dB ref 25.4 \mu mm/s]', 'FontName', 'Times New Roman', ...
       'FontSize', 20, 'Color', [0,0,0]);
% Add black box around the plot
set(gca, 'Box', 'on', 'LineWidth', 1.2);
grid on
